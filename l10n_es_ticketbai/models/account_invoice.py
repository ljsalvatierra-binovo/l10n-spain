...
class AccountInvoice(models.Model):
    ...
    tbai_previous_invoice_id = fields.Many2one(
        comodel_name='account.invoice', compute='_compute_tbai_previous_invoice',
        store=True)
        ...
        for record in self:
            if record.tbai_enabled and 'draft' == record.state and record.number:
                raise exceptions.ValidationError(_(
                    "Invoice %s. You cannot modify a numbered cancelled invoice!"
                ) % record.number)
        ...
        for record in self:
            ...
            if previous_invoice_number:
                previous_invoice = self.search(
                    [('number', '=', previous_invoice_number)])
    ...
    def tbai_build_encadenamiento_factura_anterior(self):
        if self.tbai_previous_invoice_id and \
                self.tbai_previous_invoice_id.tbai_signature_value:
            res = self
        else:
            res = None
        return res
    ...
    def tbai_get_value_serie_factura_anterior(self):
        ...
        if prefix and 20 < len(prefix):
            _logger.warning(_(
                "Previous Invoice Number Prefix for invoice %s longer than expected, "
                "20 characters max!"
            ) % self.number)
        elif prefix and 20 >= len(prefix):
    ...
    def tbai_get_value_num_factura_anterior(self):
        ...
        if 20 < len(previous_invoice_number):
            raise exceptions.ValidationError(_(
                "Previous Invoice Number %s longer than expected, 20 characters max!"
    ...
    def tbai_get_value_fecha_expedicion_factura_anterior(self):
        ...
        :return:
        """
        previous = self.tbai_previous_invoice_id
        invoice_date = previous.date or previous.date_invoice
    ...
    def tbai_get_value_signature_value_firma_factura_anterior(self):
        ...
        if self.self.tbai_previous_invoice_id:
            res = self.tbai_previous_invoice_id.tbai_signature[:100]
        else:
            res = None
        return res
